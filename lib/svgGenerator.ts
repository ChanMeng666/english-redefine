...
export const generateSVGCard = ({
    ...
    const calculateOptimalFontSizes = () => {
        ...
        const availableHeight = height - 230; // 扣除品牌标题、词汇标题、分隔线和页脚的空间
    ...
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- 品牌标题 -->
      <text x="40" y="45" font-family="'Georgia', 'Times New Roman', serif" font-size="16" font-style="italic" fill="#94a3b8">
        English Redefine
      </text>
      ...
      ${(() => {
        ...
        if (titleWidth > availableWidth) {
          ...
          return titleLines.slice(0, 2).map((line, index) => `
            <text x="40" y="${90 + index * (titleFontSize * 1.2)}" font-family="'PingFang SC', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial" font-size="${titleFontSize}" font-weight="bold" fill="#1e293b">
              ${line}${index === 1 && titleLines.length > 2 ? '...' : ''}
            </text>
          `).join('');
        } else {
          return `
            <text x="40" y="110" font-family="'PingFang SC', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial" font-size="${titleFontSize}" font-weight="bold" fill="#1e293b">
              ${word}
            </text>
          `;
        }
      ...
      ${(() => {
        const titleWidth = getTextWidth(word, titleFontSize);
        const availableWidth = width - 80;
        const separatorY = titleWidth > availableWidth ? 150 : 130;
      ...
      ${(() => {
        const titleWidth = getTextWidth(word, titleFontSize);
        const availableWidth = width - 80;
        const textStartY = titleWidth > availableWidth ? 200 : 180;
